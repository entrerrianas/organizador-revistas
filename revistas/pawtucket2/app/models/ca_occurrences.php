...
class ca_occurrences extends RepresentableBaseModel implements IBundleProvider {
	...
	public function getHierarchyPlacesName($pn_id=null) {
		if (!$pn_id) { $pn_id = $this->getPrimaryKey(); }
		$va_related_places = $this->get('ca_places.place_id', array('returnAsArray' => true));
		if (is_array($va_related_places) && sizeof($va_related_places)) {
			return $va_related_objects;
							}
						}
