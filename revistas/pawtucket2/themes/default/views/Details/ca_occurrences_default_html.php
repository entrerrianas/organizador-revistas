...
<?php	
			...
			print "</h2>";
			print "<div class='detailSubtitle'>".$t_item->get('ca_occurrences.contentVisitados.lugaresvisitados.displayName')."</div>"; 
									
			print "<h3>".'Revista: '.caNavLink($this->request, $t_item->get('ca_objects.preferred_labels.name'), '', 'Detail', 'Objects/'.$t_item->get('ca_objects.object_id'))."</h3>";
			
			#---------------sofia comienaza agregado
			#print ($t_item->get('ca_places.preferred_labels.name').' '.$t_item->get('ca_places.place_id'));
			$va_places = $t_item->get('ca_places',array('returnAsArray' => true));
			
			if (sizeof($va_places) > 0) {
				print 'Lugares: ';
				print "<div id='mediaArea'>";
					
					foreach ($va_places as $place_id => $va_place) {
						print "<div class='imageSet'>";
						#print_r($va_places);
						$va_place_id = $va_place['place_id'];
						$t_place = new ca_places($va_place_id);
								
						
						print $t_place->get('ca_places.parent.preferred_labels.name').', '.$t_place->get('ca_places.preferred_labels');
						print $t_place->get('ca_list_items.children.preferred_labels.name_plural');
						print $t_place->get('ca_places.hierarchy ');
...
<?php
	...
	if (sizeof($va_entities) > 0) {
		...
		print "<div class='blockTitle'>"._t('Autores/Fotógrafos')."</div>";
			print "<div class='blockResults'>";
				print "<div style='width:100000px'>";
				$vn_i = 0;
				foreach ($va_entities as $entity_id => $va_entity) {
					...
					if ($va_entity['item_type_id'] == 72){$data= "Autor: ";}
								elseif($va_entity['item_type_id'] == 73) 
								{$data= "Fotógrafo: ";}
								
					print $data."<div>".caNavLink($this->request, $va_entity['displayname'], '', '','Detail', 'Entities/'.$va_entity['idno'])."</div>";
					print "</div>";
					$vn_i++;
					if ($vn_i == 5) {				
						print "</div>";
